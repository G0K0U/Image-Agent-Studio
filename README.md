# 🌸 Anima Agent Studio (双引擎智能工作流控制台)
...
- **严格显存接力（0 冲突，0 爆显存）**：
  在 16GB 显存环境下，Agent 大模型（27B）按需调入内存与短上下文（-c 4096）进行规划推理（约 5 秒），规划完毕后**立即释放卸载**，将 100% 显卡显存完全让渡给 ComfyUI 满血渲染。
- **双模态智能提示词重写引擎**：
...
## 🤖 交给 AI Agent 一键部署 (Agent Quickstart)
